...
/* ═══════════════════════════════════════════════════════════════════════════
   FLOATING STICKY-NOTE SIDEBAR
   Strategy:
   - Move the entire Streamlit sidebar off-screen left when collapsed
   - When expanded it slides in as a floating panel (fixed position)
   - The toggle chevron button becomes the sticky "tab" visible at all times
   - Main content always uses full width (no sidebar gap)
═══════════════════════════════════════════════════════════════════════════ */

/* ── Hide the default sidebar toggle arrow that Streamlit renders ── */
button[data-testid="baseButton-headerNoPadding"] {
    display: none !important;
}

/* ── Make the sidebar a floating fixed panel ── */
[data-testid="stSidebar"] {
    ...
    box-shadow: 4px 0px 24px rgba(11,83,148,0.22), 2px 0 8px rgba(0,0,0,0.10) !important;
    background: linear-gradient(160deg, #0B5394 0%, #1a6fba 60%, #e8f0f8 100%) !important;
    transition: transform 0.32s cubic-bezier(.4,0,.2,1), box-shadow 0.3s !important;
    overflow: hidden !important;
    padding: 0 !important;
}

/* Collapsed state — slide completely off screen to the left */
[data-testid="stSidebar"][aria-expanded="false"] {
    transform: translateX(-310px) !important;
}

/* Expanded state — slide fully into view */
...
/* ── Sidebar text / label colours (white on dark bg) ── */
[data-testid="stSidebar"] * {
...
/* ── Floating toggle tab — always visible at left edge ── */
...
/* The actual collapse/expand button */
[data-testid="stSidebarCollapsedControl"] button {
    position: relative !important;
    width: 36px !important;
    height: 110px !important;
    background: linear-gradient(180deg, #F4A300 0%, #e09000 100%) !important;
...
[data-testid="stSidebarCollapsedControl"] button:hover {
    background: linear-gradient(180deg, #ffb700 0%, #F4A300 100%) !important;
    box-shadow: 4px 3px 16px rgba(0,0,0,0.30) !important;
}

/* Hide Streamlit's default SVG chevron icon inside the button */
[data-testid="stSidebarCollapsedControl"] button svg {
    display: none !important;
}

/* Inject vertical "📋 CRITERIA" text via pseudo-element */
...
/* When sidebar is expanded, shift the tab to sit flush against the panel edge */
[data-testid="stSidebar"][aria-expanded="true"]
    ~ [data-testid="stSidebarCollapsedControl"] {
...
/* ── Main content always full width — no sidebar gap ── */
[data-testid="stSidebar"] ~ .main,
[data-testid="stSidebar"] ~ div > .main,
...
div[data-testid="stCaptionContainer"] p {
    background: linear-gradient(90deg,rgba(11,83,148,0.08),rgba(244,163,0,0.08));
